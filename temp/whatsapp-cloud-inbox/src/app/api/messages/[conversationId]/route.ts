    ...
    if (!phoneNumberId && authHeader && userId && appUrl) {
      try {
        ...
        if (response.ok) {
          ...
          console.warn(
            '⚠️ [MessagesByConversation] No se pudo obtener phone_number_id desde appUrl:',
            response.status,
            response.statusText
          );
        }
      } catch (error) {
        console.error('❌ [MessagesByConversation] Error consultando appUrl para phone_number_id:', error);
      }
    }
