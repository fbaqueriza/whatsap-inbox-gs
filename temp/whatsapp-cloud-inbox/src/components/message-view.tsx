...
export function MessageView({ conversationId, phoneNumber, contactName, onTemplateSent, onBack, isVisible = false }: Props) {
  const { phoneNumberId, authToken, appUrl, userId } = useInboxConfig();
  ...
  const fetchMessages = useCallback(async () => {
    if (!conversationId || (!phoneNumberId && !userId)) return;
    ...
    try {
      ...
      const response = await fetch(`/api/messages/${conversationId}?${query.toString()}`, {
  ...
  useAutoPolling({
    interval: 5000,
    enabled: !!conversationId && (!!phoneNumberId || !!userId),
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if ((!messageInput.trim() && !selectedFile) || !phoneNumber || sending || (!phoneNumberId && !userId)) return;
  ...
  const handleMessageKeyDown = (event: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (sending || (!messageInput.trim() && !selectedFile)) {
          ...
          <>
            ...
            <form onSubmit={handleSendMessage} className="p-3 max-w-[900px] mx-auto w-full flex gap-2 items-center">
              ...
              <Textarea
                ...
                disabled={sending}
                className="flex-1 bg-white border-[#d1d7db] focus-visible:ring-[#00a884] rounded-lg resize-none leading-5"
              />
              <Button
                type="submit"
