...
export function TemplateSelectorDialog({ open, onOpenChange, phoneNumber, onTemplateSent }: Props) {
  const { phoneNumberId, wabaId, userId, appUrl, authToken } = useInboxConfig();
  ...
  const fetchTemplates = async () => {
    if (!wabaId) {
      setError('Falta WABA_ID para obtener los templates disponibles.');
      setTemplates([]);
      setLoading(false);
      return;
    }
    ...
    setError(null);
    try {
      const response = await fetch(`/api/templates?wabaId=${encodeURIComponent(wabaId)}`);
  ...
  const handleSendTemplateWithoutParameters = async (template: Template) => {
    if (!phoneNumberId) {
      setError('Falta phoneNumberId para enviar templates.');
      return;
    }
