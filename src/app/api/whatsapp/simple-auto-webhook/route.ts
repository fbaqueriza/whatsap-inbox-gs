...
export async function POST(request: NextRequest) {
  try {
    console.log('🚀 [SimpleAutoWebhook] Configurando webhook automáticamente...');

    const body = await request.json();
    const { kapsoConfigId = 'bae605ec-7674-40da-8787-1990cc42cbb3' } = body;
    ...
    console.log('🔧 [SimpleAutoWebhook] Configurando webhook en Kapso:', {
      kapsoConfigId,
      webhookUrl,
    ...
    console.log('📊 [SimpleAutoWebhook] Respuesta de Kapso:', {
    ...
    console.error('❌ [SimpleAutoWebhook] Error:', error);
    return NextResponse.json({
      success: false,
      error: error.message
    }, { status: 500 });
